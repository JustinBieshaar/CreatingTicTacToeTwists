...
public class UltimateGrid implements IGameObject {
	
	private ArrayList<UltimatePlacement> placements = new ArrayList<UltimatePlacement>(Main.SIZE);
	...
	public UltimateGrid(int x, int y, int size, boolean drawGrid, boolean drawActive) {
		this(x, y, size);
	...
	public UltimateGrid(int x, int y, int size) {
		this.size = size;
		this.x = x;
		this.y = y;
		...
		for (int i = 0; i < Main.SIZE; i++) {
			...
			placements.add(new UltimatePlacement(startX + xIndex * cellSize, startY + yIndex * cellSize, xIndex, yIndex, cellSize, cellSize));
		}
	...
	public void update(float deltaTime) {
		for (UltimatePlacement placement : placements) {
	...
	public UltimatePlacement mouseReleased(MouseEvent e, int markerIndex) {
		setMarkerIndex(markerIndex);
		return mouseReleased(e);
	}

	public UltimatePlacement mouseReleased(MouseEvent e) {
		for (UltimatePlacement placement : placements) {
